...
def calculate_open_gates_percentage(gates):
    ...
    num_open_gates = gates.sum()
    total_gates = gates.numel()  # Total number of gates in the tensor
...
def write_tblog(tblogger, epoch, results, lrs, losses, gates=None):
    ...
    if len(losses) >= 4:
        ...
        tblogger.add_histogram('Percentage Open Gates by Layer', np.array(percentage_open_gates), global_step=epoch + 1, bins=len(np.array(percentage_open_gates)))
